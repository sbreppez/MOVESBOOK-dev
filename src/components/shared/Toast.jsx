...
export const Toast = ({ toasts, remove }) => (
  <div style={{ position:"fixed", top:68, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, zIndex:9999, display:"flex", flexDirection:"column", alignItems:"flex-end", gap:8, pointerEvents:"none", padding:"0 10px", boxSizing:"border-box" }}>
    {toasts.map(t => (
      <div key={t.id} style={{ background: C.surface, borderRadius:8, padding:"11px 14px", display:"flex", alignItems:"flex-start", gap:10, pointerEvents:"all", animation:"toastIn 0.3s ease" }}>
        <span style={{ flexShrink:0, display:"flex", alignItems:"center" }}><Ic n={t.icon||"swords"} s={16} c={C.accent}/></span>
        <div style={{ flex:1 }}>
