            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {allCatMoves.map((m,idx)=>(
                <div key={m.id} style={{ position:"relative" }}>
                  ...
                  <MoveListRow move={m} catColor={catColor} onEdit={()=>setEditMove(m)} onDelete={()=>delMove(m.id)} onMove={cat=>moveToCat(m.id,cat)} allCats={cats} catColors={catColors}/>
                </div>
              ))}
            </div>
          ) : catView==="tiles" ? (
