...
export const MoveListRow = ({ move, catColor, onEdit, onDelete, onMove: _onMove, allCats: _allCats, catColors: _catColors, selectMode, isSelected }) => {
  ...
  return (
    <div onClick={onEdit} style={{ position:"relative", display:"flex", alignItems:"center", gap:10, background:C.surface, borderRadius:8, padding:"10px 12px", borderLeft:`4px solid ${catColor}`, cursor:"pointer",
      ...
      {selectMode && (
        <div style={{ position:"absolute", top:"50%", left:10, transform:"translateY(-50%)", width:20, height:20, borderRadius:5,
      ...
      {!selectMode&&<button onClick={e=>{e.stopPropagation();onDelete();}}
