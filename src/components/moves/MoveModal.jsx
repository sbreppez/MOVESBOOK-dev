...
export const MoveModal = ({ onClose, onSave, move, initialCat="Footworks", initialDesc="", cats=CATS }) => {
  ...
  return (
    <Modal title={move?t("editMove"):t("addMove")} onClose={onClose}>
      ...
      <Inp label={t("name") + " *"} value={f.name} onChange={set("name")} placeholder="Move name…"/>
      ...
      {more && <div style={{ background:C.surfaceAlt, borderRadius:10, padding:14, marginBottom:16, border:`1px solid ${C.borderLight}` }}>
        <Sel label={t("direction")} value={f.rotation} onChange={set("rotation")} options={[{value:"",label:"None"},"Clockwise","Counterclockwise"]}/>
        <Sel label={t("travelling")} value={f.travelling} onChange={set("travelling")} options={[{value:"",label:"None"},"Forward","Backwards","Sideways","Going Around"]}/>
      </div>}
