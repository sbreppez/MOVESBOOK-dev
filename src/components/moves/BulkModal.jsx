...
export const BulkModal = ({ onClose, onImport, cats }) => {
  ...
  const imp=()=>{ onImport(parsed.filter(p=>sel.includes(p.id)).map(p=>({name:p.name,category:cat,mastery,description:"",link:"",date:todayLocal(),createdAt:todayLocal(),status:"wip"}))); onClose(); };
