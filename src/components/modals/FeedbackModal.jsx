...
export const FeedbackModal = ({ onClose }) => {
  ...
  const [vocabLikes, setVocabLikes] = useState([]);
  const [vocabOther, setVocabOther] = useState("");
  const [vocabImp,   setVocabImp]   = useState("");
  ...
  const handleSubmit = async () => {
    ...
    const body = [
      ...
      "",
      buildSection("VOCAB SECTION:",  vocabLikes,  vocabOther,  vocabImp),
      "",
          ...
          <div style={{ marginBottom:22 }}>
            ...
            {VOCAB_OPTIONS.map(o=>(
              <button key={o} onClick={()=>toggleCheck(o,vocabLikes,setVocabLikes)}
                style={{ display:"flex", alignItems:"center", gap:10, width:"100%", background:"none", border:"none", cursor:"pointer", padding:"7px 0", textAlign:"left" }}>
                <div style={{ width:18, height:18, borderRadius:4, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center",
                  border:`2px solid ${vocabLikes.includes(o)?C.accent:C.border}`, background:vocabLikes.includes(o)?C.accent:"transparent", transition:"all 0.15s" }}>
                  {vocabLikes.includes(o)&&<Ic n="check" s={11} c={C.bg}/>}
                </div>
                <span style={{ fontSize:13, color:C.textSec, fontFamily:FONT_BODY }}>{o}</span>
              </button>
            ))}
