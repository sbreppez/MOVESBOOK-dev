...
export default function App() {
  ...
  const setSets = useCallback((updater) => {
    setSetsState(prev => {
      const next = typeof updater === 'function' ? updater(prev) : updater;
      if (fbUser?.uid) {
        emitSetsChanges(prev, next, fbUser.uid).catch(err => {
          console.error('[textStream] sets emit failed:', err);
        });
      }
      return next;
    });
  ...
  const setBattleprep = useCallback((updater) => {
    setBattleprepState(prev => {
      const next = typeof updater === 'function' ? updater(prev) : updater;
      if (fbUser?.uid) {
        emitBattleprepChanges(prev, next, fbUser.uid).catch(err => {
          console.error('[textStream] battleprep emit failed:', err);
        });
      }
      return next;
    });
    ...
    function handleAuthResolved() {
      if (window.__MB_USER__) {
        ...
        try {
          ...
          if (s) { try { const p=JSON.parse(s); if(Array.isArray(p)&&p.length>0) setSetsState(p); } catch {} }
  ...
  const updateCalendarEvent = useCallback((updatedEvent) => {
    setCalendar(prev => ({
      ...prev,
  ...
  const dispatchJumpToSource = useCallback((sourceType, sourceId) => {
    ...
    switch (category) {
      ...
      case 'sets':
        ...
        break;
      case 'calendar': {
        // Phase 2: jump-to-source for calendar events lands on HOME at the
        // event's day. Scroll-to-tile (eventId focus) is a follow-up issue.
        const event = (calendar?.events || []).find(e => String(e.id) === String(primaryId));
        const day = toYMD(event?.date);
        if (day) {
          setHomeSeed({ kind: 'day', day });
          setTab('home');
        }
        break;
      }
      case 'sessions': {
        // Phase 2: jump-to-source for sessions lands on HOME at the session's
        // day. Scroll-to-tile (sessionKind/sessionId focus) is a follow-up.
        // session.date may be an ISO timestamp or a bare YMD; toYMD normalizes.
            ...
            onSaveSession={(session, updatedSparring)=>{
              ...
              if(session.movesTrained?.length){
                setMovesState(prev=>{
                  ...
                  return session.movesTrained.reduce((acc,id)=>appendTrainingEntry(acc,id,{ date:d, count:0, source:'sparring' }),prev);
                });
              }
              lastSessionSaved.current=true;
            }}
