...
export default function App() {
  ...
  // ── Migrate pinnedNotes → pinnedHome (one-time, idempotent) — Session L3a ──
  useEffect(() => {
    try {
      ...
      localStorage.setItem("mb_pinned_notes_migrated", "1");
    } catch (e) {
      console.warn("[MB] pinnedNotes migration failed:", e);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps -- one-shot mount migration; mb_pinned_notes_migrated localStorage flag enforces idempotence
  }, []);
      ...
      <div id="movesbook-root" style={{ fontFamily:FONT_BODY, background:C.bg, color:C.text,
        ...
        <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column", position:"relative" }}>
          ...
          <TrainModalCtx.Provider value={{ openModal:(type,idea,onSave)=>{ setTrainModal({type,idea,onSave}); } }}>
            {tab==="home" && !showCreate && <HomePage habits={habits} setHabits={setHabits} injuries={injuries} setInjuries={setInjuries} restLog={restLog} setRestLog={setRestLog} restTypes={restTypes} setRestTypes={setRestTypes} presession={presession} setPresession={setPresession} ideas={ideas} setIdeas={setIdeas} settings={appSettings} onSettingsChange={setAppSettings} homeStack={homeStack} setHomeStack={setHomeStack} homeChecks={homeChecks} setHomeChecks={setHomeChecks} onAddTrigger={addTick} addCalendarEvent={addCalendarEvent} removeCalendarEvent={removeCalendarEvent} calendar={calendar} moves={moves} setMoves={setMovesGrad} cats={cats} catColors={catColors} sets={sets} recordEventTraining={recordEventTraining} updateCalendarEvent={updateCalendarEvent} customAttrs={customAttrs} setCustomAttrs={setCustomAttrs} isPremium={isPremium} addToast={addToast} homeSeed={homeSeed} onHomeSeedUsed={()=>setHomeSeed(null)} setBattles={setBattles} battleFormats={battleFormats} setBattleFormats={setBattleFormats}/>}
